...
function underscores_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Published %s', 'post date', 'underscores' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( 'Written by %s', 'post author', 'underscores' ),
...
function underscores_the_category_list() {
    /* translators: used between list items, there is a space after the comma */
    $categories_list = get_the_category_list( esc_html__( ', ', 'underscores' ) );
    if ( $categories_list && underscores_categorized_blog() ) {
        printf( '<span class="cat-links">' . esc_html__( '%1$s', 'underscores' ) . '</span>', $categories_list ); // WPCS: XSS OK.
    }
}
