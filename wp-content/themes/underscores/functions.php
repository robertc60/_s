...
function underscores_fonts_url() {
	 ...
	 */
	$source_sans_pro = _x( 'on', 'Source Sans Pro font: on or off', 'underscores' );
	$pt_serif = _x( 'on', 'PT Serif font: on or off', 'underscores' );
 ...
 */
function underscores_resource_hints( $urls, $relation_type ) {
	if ( wp_style_is( 'underscores-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
		$urls[] = array(
...
add_filter( 'wp_resource_hints', 'underscores_resource_hints', 10, 2 );
...
function underscores_scripts() {
	...
	wp_enqueue_style( 'underscores-fonts', underscores_fonts_url() );
